...
%token INCREMENT DECREMENT INPUT PRINTF SCANF
...
%type <content> scanf_stat
%type <content> printf_stat
...
complete_program:headerFilesMacros starter
		;
headerFilesMacros:headerFilesMacros FilesMacros 
		 |
		 ;
FilesMacros:headerFiles 
	   |headerMacros 
	   ;
headerFiles:HASH_INCL IDENTIFIER HASH_INCL_END
	   ;
headerMacros:HASH_DEF IDENTIFIER macroExpr
	   ;

macroExpr : IDENTIFIER
	| constant
	| '(' sub_expr ')'
	;
...
single_stmt :if_block	{
			...
			|scanf_stat ';'		{$$ = new content_t();}
			|printf_stat ';'		{$$ = new content_t();}
			|error			{$$ = new content_t();}
	    ;

scanf_stat: SCANF '(' STRING ')'		{$$ = new content_t();}
	;
printf_stat:PRINTF '(' STRING ')'	{$$ = new content_t();}
	;
	...
	lhs '=' arithmetic_expr	
			{
				if (lhs_array == 0) {
					...
					$$->code = $1->entry->lexeme + string("[") + $1->addr + string("]") + string(" = ") + $3->addr;
	...
	lhs '=' array_access
			{
				...
				$$->data_type = $3->data_type;
	 			...
	 			$$->code = string($1->entry->lexeme) + " = " + string("t")+to_string(temp_var_number-1);
...
lhs: identifier		{$$ = new content_t(); $$->entry = $1; lhs_array = 0;}

   | array_access	{$$ = new content_t(); 
   			$$->code = $1->code; 
   			$$->data_type = $1->data_type;
...
array_access: identifier '[' array_index ']'					
				{
					...
					if($3->is_constant && !is_declaration)
					{
						//printf("is lhs array = %d\n", lhs_array);
						//printf("%s is constant and is not a declaration\n", $3->lexeme);
						$$->addr = "t" + to_string(temp_var_number);
						temp_var_number++;
						std::string expr = $$->addr + " = " + "4 * " + to_string($3->value);
						
						
						std::string expr1 = "t" + to_string(temp_var_number) + " = "+ string($1->lexeme)+string("[") + $$->addr + string("]");
						/*
						printf("\nEXPR\n");	
						cout<<expr;
						printf("\nEXPR1\n");		
						...
						*/
						...
						$$->code = expr ; //+string($$->entry->lexeme)+" = "+to_string(temp_var_number) + "\n";
						gencode($$->code);
						$$->code= expr1;
						gencode($$->code);
						$$->code = expr + expr1;
