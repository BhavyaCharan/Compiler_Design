...
%token <node> MUL_EQ DIV_EQ MOD_EQ ADD_EQ SUB_EQ
%token <node> INCREMENT DECREMENT

%token <node> SHORT INT LONG SIGNED UNSIGNED CONST FLOAT VOID

%token <node> IF FOR DO WHILE CONTINUE BREAK RETURN PRINTF SCANF
...
%type <node> statement statements compound_statement single_statement printf_statement scanf_statement
%type <node> ifBlock forBlock whileBlock
...
headerFiles:HASH_INCL ID_HI				{$$ = newnode("Hash_include", 0, 2, $1, $2);}
	   ;
ID_HI:IDENTIFIER HASH_INCL_END				{$$ = newnode("Identifier", 0, 2, $1, $2);}
	;

headerMacros:HASH_DEF ID_ME 				{$$ = newnode("headerMacros", 0, 2, $1, $2);}
	   ;
ID_ME: IDENTIFIER macroExpr				{$$ = newnode("ID_ME", 0, 2, $1, $2);}
	;
...
type_specifier :INT                    {DECL = 1;current_dtype = 1;current_storage=4; $$ = newnode("int", 0, 2, NULL, NULL);}
    ...
    |VOID			       {DECL = 1;current_dtype = 5;current_storage=0; $$ = newnode("void", 0, 2, NULL, NULL);}
    ;
...
printf_statement: PRINTF '(' print_inside ')'		{$$ = newnode("printf_statement", 0, 2, NULL, NULL);}
;

print_inside: STRING ',' print_args
		| STRING
		;
...
scanf_statement: SCANF '(' scanf_inside ')'		{$$ = newnode("scanf_statement", 0, 2, NULL, NULL);}
;

scanf_inside: STRING ',' scanf_args
		;

scanf_args: '&'IDENTIFIER
	| scanf_args ',' scanf_args
	| IDENTIFIER
	;
